...
class PhotometricFitting(object):
    ...
    def run(self, img, vis_folder, existing_params = None, imgmask_path = None, config = None, imglmark = None ):
        ...
        if existing_params:
            shape = existing_params['shape']#[1,100]
            exp = existing_params['exp'] #[1,50]
            # pose = existing_params['pose'] #[1,6]
            # cam = existing_params['cam'] #[1,3]
            tex = existing_params['tex'] #[1,50]
            lit = existing_params['lit'] #[1,9,3]
            
            shape = torch.FloatTensor(shape).to(self.device)
            exp = torch.FloatTensor(exp).to(self.device)
            # pose = torch.FloatTensor(pose).to(self.device)
            # cam = torch.FloatTensor(cam).to(self.device)
            tex = torch.FloatTensor(tex).to(self.device)
            lights = torch.FloatTensor(lit).to(self.device)
            # optimize
            single_params = self.optimize(images, landmarks , image_masks, savefolder= vis_folder, show = True, shape =shape , exp = exp, tex = tex, lights =lights)
        else:
...
def main_ffhq(config):
    ...
    num_files, img_names = open_image_folder(config.imgfolder,max_images = max_images)
